...
import { Analytics } from '@/services/MID-API'
...
const getCount = async () => {
  await Analytics.fetchRecords()
    .then((response) => {
      const total = response.data.total
      cardItems.value[0].total = total.companies;
      cardItems.value[1].total = total.members;
      cardItems.value[2].total = total.entries;
      cardItems.value[3].total = total.inquiries;
      
      // console.log('getCount:', response.data.total);
    });
};
